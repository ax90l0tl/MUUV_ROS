...
double PID::compute(double setpoint, double state, double dt)
{
    ...
    {
        return compute(CommonFunctions::AngleDiffRadians(state, setpoint), dt); // state-setpoint because of sign convention for AngleDiffRadians
    }
    else
    {
...
double PID::compute(double error, double dt)
{
    ...
    control_effort = CommonFunctions::Clamp(control_effort, -output_limit_, output_limit_);
    // cout << "control " << control_effort << endl;
